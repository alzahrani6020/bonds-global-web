...
module.exports = async function handler(req, res) {
  ...
  if (cached) {
    res.status(cached.allHealthy ? 200 : 503).json(cached.payload);
    return;
  }
  ...
  // Check Supabase and Stripe in parallel
  const checks = [];
  ...
  // Cache health status for 10 seconds to reduce DB load
  setCache(cacheKey, { allHealthy, payload }, 10000);
