  ...
  if (req.method === 'GET') {
    try {
      ...
      if (isMissingTableError(err)) {
        console.warn('[SocialSchedule] scheduled_posts table not ready, returning empty list');
        return sendJson(res, 200, { success: true, posts: [], skipped: true, reason: 'social_scheduled_posts table not ready' });
      }
...
function isMissingTableError(err) {
  ...
  return /Could not find the table|relation .* does not exist|42P01|PGRST204/i.test(msg);
}
...
async function handleCron(req, res) {
  ...
  try {
    ...
    if (isMissingTableError(err)) {
      console.warn('[SocialCron] scheduled_posts table not ready, skipping gracefully');
      return sendJson(res, 200, { success: true, processed: 0, skipped: true, reason: 'social_scheduled_posts table not ready' });
    }
