/**
 * BONDS Server-side Validation Layer
 * Reusable, strict input validation for Vercel serverless APIs.
 */
...
function validateField(name, value, rules) {
  ...
  if (rules.required && !validators.required(value)) {
    errors.push({ field: name, code: 'required', message: `Field ${name} is required.` });
    return errors;
  }
  if (!validators.required(value)) return errors;

  if (rules.type && validators[rules.type] && !validators[rules.type](value)) {
    errors.push({ field: name, code: 'type', message: `Field ${name} must be of type ${rules.type}.` });
  }
  if (rules.email && !validators.email(value)) {
    errors.push({ field: name, code: 'email', message: `Field ${name} must be a valid email.` });
  }
  if (rules.uuid && !validators.uuid(value)) {
    errors.push({ field: name, code: 'uuid', message: `Field ${name} must be a valid UUID.` });
  }
  if (rules.phone && !validators.phone(value)) {
    errors.push({ field: name, code: 'phone', message: `Field ${name} must be a valid Saudi or international phone.` });
  }
  if (rules.priceId && !validators.priceId(value)) {
    errors.push({ field: name, code: 'priceId', message: `Field ${name} must start with price_.` });
  }
  if (rules.countryCode && !validators.countryCode(value)) {
    errors.push({ field: name, code: 'countryCode', message: `Field ${name} must be an ISO-3166 two-letter country code.` });
  }
  if (rules.status && !validators.status(value, rules.status)) {
    errors.push({ field: name, code: 'status', message: `Field ${name} must be one of: ${rules.status.join(', ')}.` });
  }
  if (rules.minLength !== undefined && !validators.minLength(value, rules.minLength)) {
    errors.push({ field: name, code: 'minLength', message: `Field ${name} must be at least ${rules.minLength} characters.` });
  }
  if (rules.maxLength !== undefined && !validators.maxLength(value, rules.maxLength)) {
    errors.push({ field: name, code: 'maxLength', message: `Field ${name} must be at most ${rules.maxLength} characters.` });
  }
  if (rules.min !== undefined && validators.number(value) && value < rules.min) {
    errors.push({ field: name, code: 'min', message: `Field ${name} must be at least ${rules.min}.` });
  }
  if (rules.max !== undefined && validators.number(value) && value > rules.max) {
    errors.push({ field: name, code: 'max', message: `Field ${name} must be at most ${rules.max}.` });
  }
  if (rules.oneOf && !validators.oneOf(value, rules.oneOf)) {
    errors.push({ field: name, code: 'oneOf', message: `Field ${name} must be one of: ${rules.oneOf.join(', ')}.` });
  }
  if (rules.custom && typeof rules.custom === 'function') {
    const custom = rules.custom(value);
    if (custom !== true) errors.push({ field: name, code: 'custom', message: custom || `Field ${name} is invalid.` });
  }
  return errors;
}
