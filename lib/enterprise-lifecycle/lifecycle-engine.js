...
class LifecycleEngine {
  ...
  async buildContext(instance, baseContext = {}) {
    ...
    // Always include tasks and approvals so gates can reference them.
    if (!context.tasks && this.store) {
      try {
        context.tasks = await this.store.listTasks(instance.id);
      } catch (err) {
        context.tasks = [];
      }
    }
    if (!context.approvals && this.store) {
      try {
        const approvals = await this.store.listApprovals(instance.id);
        context.approvals = approvals.reduce((map, a) => {
          map[a.transition_id || a.id] = a;
          return map;
        }, {});
      } catch (err) {
        context.approvals = {};
  ...
  async completeTask({ taskId, userId, evidence = [], context = {} }) {
    await this.initialize();
    const result = await this.taskEngine.completeTask({
      taskId,
      store: this.store,
      completedBy: userId,
      evidence,
    ...
    if (result.success) {
      ...
      await this.eventBus.emitAndStore({
        ...
        payload: { taskId: task.id, taskCode: task.task_code, completedBy: userId },
        source: 'lifecycle-engine',
        store: this.store
      });
      await this.timelineEngine.addEntry({
        instanceId: task.instance_id,
        entryType: 'task',
        title: `Task completed: ${task.title || task.task_code}`,
        description: `Task ${task.task_code} completed by ${userId}`,
        actor: userId,
        referenceTable: 'enterprise_lifecycle_tasks',
        referenceId: task.id,
        store: this.store
      });
    }
