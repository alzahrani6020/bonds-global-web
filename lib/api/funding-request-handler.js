...
async function createFundingCase(sb, body, idempotencyKey, user) {
...
async function handleFundingExtractionRequest(req, res, user) {
  if (req.method !== 'POST') {
  ...
  try {
    ...
    const caseResult = await createFundingCase(sb, body, idempotencyKey, user);
...
module.exports = { handleFundingExtractionRequest, createFundingCase };
