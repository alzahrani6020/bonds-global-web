...
const LIMITS = {
  global: { limit: parseInt(process.env.RATE_LIMIT_GLOBAL_PER_MIN || '200', 10), windowMs: 60_000 }, // all /api/platform requests
  ...
  bank_transfer: { limit: 5, windowMs: 60_000 }, // public bank-transfer submissions
  live: { limit: 60, windowMs: 60_000 },         // real-time dashboards / telemetry
  webhook: { limit: 1000, windowMs: 60_000 },   // Stripe/Moyasar webhooks
  funding_case_guest_lookup_global: { limit: 60, windowMs: 60_000 },
  funding_case_guest_lookup_per_case: { limit: 10, windowMs: 60_000 },
};

const store = new Map();

function getWindowKey(identity, category, windowMs) {
  const bucket = Math.floor(Date.now() / windowMs);
  return `${identity}:${category}:${bucket}`;
}
...
function checkRateLimitLocal(category, cfg, req, res, identity) {
  const rateIdentity = identity || getClientIp(req);
  const key = getWindowKey(rateIdentity, category, cfg.windowMs);
 ...
 */
async function checkRateLimit(category, req, res, identity, failClosed = false) {
  if (!LIMITS[category]) {
  ...
  const distributed = await checkRateLimitDistributed(category, cfg, req, res, identity, failClosed);
  if (typeof distributed === 'boolean') {
    return distributed;
  }

  if (failClosed) {
    // Distributed backend could not provide a definite allow; fail closed.
    res.setHeader('Retry-After', String(Math.ceil(cfg.windowMs / 1000)));
    res.status(429).json({ error: 'Too many requests. Please try again later.' });
    return true;
  }

  return checkRateLimitLocal(category, cfg, req, res, identity);
}
