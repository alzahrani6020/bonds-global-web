...
const ECC_ADVISOR_SCHEMA = `
أخرج JSON بالهيكل التالي فقط:
{
  "reply": "رد ودي ومختصر بالعربية (2-4 جمل) يجيب على سؤال المستخدم",
  "action": "الإجراء المقترح التالي بشكل قصير",
  "action_ar": "الإجراء المقترح بالعربية",
...
- إذا كانت البيانات غير كافية لإجابة دقيقة، قل "بيانات غير متوفرة" واشرح ما هو ناقص.
- confidence رقم 0-100 يعبر عن ثقتك في الإجابة.
- لا تضف حقولاً خارج الهيكل.
`.trim();
...
const INVESTMENT_STORY_SCHEMA = `
أخرج JSON بالهيكل التالي فقط:
{
  "story": {
    "summary": "ملخص القصة الاستثمارية بجملتين",
    "whyThisProject": "لماذا هذا المشروع؟",
    "whyNow": "لماذا الآن؟",
    "whyThisMarket": "لماذا هذا السوق؟",
    "whyThisTeam": "لماذا هذا الفريق؟",
    "whyThisOpportunity": "لماذا هذه الفرصة؟",
    "whyThisValuation": "لماذا هذا التقييم؟",
    "whyThisFunding": "لماذا هذا التمويل؟"
  },
  "confidence": 0
}
...
- لا تخترع أرقاماً.
- اكتب بلغة بشرية احترافية.
- confidence رقم 0-100 تعبّر عن ثقتك في جودة القصة بناءً على اكتمال البيانات.
`.trim();

const BREAK_EVEN_ADVICE_SCHEMA = `
أخرج JSON بالهيكل التالي فقط:
{
  "executive_summary": "ملخص تنفيذي مختصر (2-3 جمل) يفهمه العميل غير التقني",
  "health_status": "ممتاز | جيد | يحتاج مراجعة | حرج",
  "key_insights": ["ملاحظة 1", "ملاحظة 2"],
  "pricing_recommendations": ["توصية تسعير 1"],
  "cost_recommendations": ["توصية تكاليف 1"],
  "risk_factors": ["مخاطر 1"],
  "next_steps": ["الخطوة التالية 1"],
  "confidence": 0,
  "missing_data": ["بيان ناقص 1"]
}

قواعد:
- اعتمد فقط على أرقام نقطة التعادل المقدمة.
- لا تخترع أرقاماً أو معلومات سوقية غير موجودة.
- اكتب بلغة عربية احترافية.
- confidence رقم 0-100.
- missing_data اتركه [] إذا كانت البيانات كافية.
`.trim();

const INVESTMENT_REVIEW_SCHEMA = `
أخرج JSON بالهيكل التالي فقط:
{
  ...
  "issues": ["وصف المشكلة 1"],
  "suggestions": ["اقتراح 1"],
  "confidence": 0
}

قواعد:
- راجع المستند كمستثمر محترف.
- لا تغير الأرقام.
- اذكر أي تضارب أو نقص أو مبالغة واضحة.
- confidence رقم 0-100.
`.trim();

const templates = {
  investment_story: (data) => `
نوع التحليل: بناء القصة الاستثمارية (Investment Story)

البيانات المدخلة:
${compactJson(data)}

المطلوب:
- اكتب قصة استثمارية مقنعة تجيب على 7 أسئلة للمستثمر.
- لا تخترع أرقاماً أو معلومات غير موجودة.
- استخدم لغة احترافية مناسبة للمستثمرين.
- إذا كانت بيانات ناقصة، اذكر ذلك بصراحة.

${INVESTMENT_STORY_SCHEMA}
`.trim(),

  break_even_advice: (data) => `
نوع التحليل: نصيحة AI لمستشار نقطة التعادل (Break-Even AI Advisor)

بيانات نقطة التعادل المدخلة:
${compactJson(data)}

المطلوب:
- قيّم صحة النموذج المالي بناءً على نقطة التعادل والهامش والربح الصافي.
- حدد ما إذا كان السعر والتكلفة والكمية متوازنة.
- اقترح تحسينات في التسعير أو التكاليف أو الكمية.
- اذكر المخاطر الرئيسية.
- حدد الخطوة التالية الأنسب.

${BREAK_EVEN_ADVICE_SCHEMA}
`.trim(),

  investment_review: (data) => `
نوع التحليل: مراجعة AI للمستند الاستثماري (AI Investment Review)
...
المطلوب:
- هل المستند مقنع؟
- هل يوجد تضارب بين الأقسام؟
- هل يوجد نقص في البيانات؟
- هل توجد مبالغة؟
- هل الأرقام منطقية؟
- هل المخاطر مذكورة؟
- هل لغة المستثمرين صحيحة؟

${INVESTMENT_REVIEW_SCHEMA}
`.trim(),
...
`.trim(),
  ...
  ecc_advisor: (data) => `
نوع التحليل: مستشار بوندز التنفيذي (AI Chief Advisor)

أنت "مستشار بوندز التنفيذي"، مساعد ذكي يقود المستخدم عبر دورة حياة مشروعه الاستثماري.

سياق المشروع الحالي:
${compactJson(data.context)}

رسالة المستخدم:
"${data.userMessage}"

المطلوب:
- أجب على سؤال المستخدم بناءً على السياق فقط.
- اشرح "ما الذي يجب فعله الآن؟" و"لماذا؟" عند الاقتباس.
- حدد الأولوية والثقة.
- لا تتخذ قرارات نهائية؛ قدّم توصيات فقط.
- لا تغير أي رقم مالي.
...
<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><path d="M12 9v4"/><path d="M12 17h.01"/></svg> قواعد صارمة:
 ...
 *
 * @param {'credit_assessment'|'feasibility_study'|'distressed_project'|'city_analysis'|'asset_valuation'|'ecc_advisor'|'break_even_advice'} type
