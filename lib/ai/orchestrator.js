...
const VALID_TYPES = ['credit_assessment', 'feasibility_study', 'distressed_project', 'city_analysis'];
...
async function analyze({
  ...
  try {
    // 6. Call AI (ONE CALL)
    const { result, tokensInput, tokensOutput } = await callOpenAI(messages, model);

    // 7. Calculate cost
    const costUsd = calculateCost(model, tokensInput, tokensOutput);
    const riskScore = result.risk_score ?? null;

    // 8. Update request with usage and status
    const supabase = getSupabase();
    await supabase
      .from('ai_requests')
      .update({
