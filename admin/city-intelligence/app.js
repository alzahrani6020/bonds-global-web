  ...
  function renderCityReportsTab(content, city) {
    ...
    const rows = reps.map(r => `
      <tr>
        ...
        <td>
          ${r.pdf_url ? `<a href="${BondsAdminCommon.escapeHtml(BondsAdminCommon.safeExternalUrl(r.pdf_url))}" target="_blank" rel="noopener noreferrer" class="ecc-btn ecc-btn--ghost ecc-btn--sm">تحميل PDF</a>` : ''}
    ...
    const rows = state.reports.map(r => {
      ...
      return `
        <tr>
          ...
          <td>${formatDate(r.created_at)}</td>
          <td>
            ${r.pdf_url ? `<a href="${BondsAdminCommon.escapeHtml(BondsAdminCommon.safeExternalUrl(r.pdf_url))}" target="_blank" rel="noopener noreferrer" class="ecc-btn ecc-btn--ghost ecc-btn--sm">تحميل</a>` : ''}
            <button class="ecc-btn ecc-btn--ghost ci-btn-danger ecc-btn--sm" onclick="CityIntelligenceApp.deleteReport('${r.id}')">حذف</button>
          </td>
        </tr>`;
    }).join('');
