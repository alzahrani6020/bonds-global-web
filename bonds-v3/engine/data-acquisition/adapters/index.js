module.exports = {
  GastatAdapter: require('./GastatAdapter'),
  SamaAdapter: require('./SamaAdapter'),
  ManualAdapter: require('./ManualAdapter'),
  LLMEstimationAdapter: require('./LLMEstimationAdapter'),
  UaeStatsAdapter: require('./UaeStatsAdapter'),
  EgyptCapmasAdapter: require('./EgyptCapmasAdapter'),
  QatarPlanningAdapter: require('./QatarPlanningAdapter'),
  JordanStatsAdapter: require('./JordanStatsAdapter'),
  CompetitorDataAdapter: require('./CompetitorDataAdapter')
};
